// semantq/auth/services/email.js
...
async send({ from, to, subject, html, text }) {
 console.log(`[ResendDriver] Sending email via Resend...`);
...
async _initializeDriver() {
 // Always load the configuration here, ensuring we get the latest
 const config = await loadConfigPromise();
 const emailConfig = config.email || {};
 const driverType = (emailConfig.driver || 'resend').toLowerCase();

 // Log what config is being used right before driver initialization
 console.log(`[EmailService] Initializing with driver: ${driverType}`);
 console.log(`[EmailService] Configured 'email_from': ${emailConfig.email_from}`);


 switch (driverType) {
 case 'resend':
 // Use whatever key is in the config; fall back to mock only if completely missing
 if (!emailConfig.resend_api_key) {
  console.warn("[EmailService] No Resend API key found. Falling back to MockEmailDriver.");
  this.driver = new MockEmailDriver();
  break;
 }

 console.log(`[EmailService] Resend Driver selected. API key is set.`);
 ...
 case 'mailgun':
  ...
  console.error("[EmailService] Mailgun config not found. Falling back to MockDriver.");
  this.driver = new MockEmailDriver();
  break;
  }
  console.log("[EmailService] Mailgun Driver selected.");
  this.driver = new MailgunDriver(emailConfig.config.mailgun);
  break;

 case 'smtp':
  if (!emailConfig.config?.smtp) {
  console.error("[EmailService] SMTP config not found. Falling back to MockDriver.");
  this.driver = new MockEmailDriver();
  break;
  }
  console.log("[EmailService] SMTP Driver selected.");
...
async sendConfirmationEmail({ to, name, token }) {
 ...
 // Using loadConfigPromise here is good for ensuring the latest base URL is used
 const config = await loadConfigPromise();
 ...
 console.log(`[EmailService] Confirmation email sent to: ${to} using ${this.driver.constructor.name}`);
 } catch (error) {
...
async sendPasswordResetEmail({ to, name, token }) {
 ...
 console.log(`[EmailService] Password reset email sent to: ${to} using ${this.driver.constructor.name}`);
 } catch (error) {
...
// EXPORT THE PROMISE FACTORY FUNCTION TO BE USED BY DYNAMIC IMPORT
export const emailServicePromise = async () => EmailService.create();