...
class ResendDriver {
  ...
  async send({ from, to, subject, html, text }) {
    ...
    try {
      ...
      if (result.error) {
        console.error(`[ResendDriver] Resend API Error:`, result.error);
        throw new Error(`Resend error: ${result.error.message}`);
      }
      
      console.log(`[ResendDriver] Email sent successfully. ID: ${result.data?.id || 'unknown'}`);
      return result;
    } catch (error) {
      console.error(`[ResendDriver] Failed to send email:`, error.message);
      throw error;
    }
  }
}
...
class MailgunDriver {
  ...
  async send({ from, to, subject, html, text }) {
    console.log(`[MailgunDriver] Sending email via Mailgun...`);
...
class SMTPDriver {
  ...
  async send({ from, to, subject, html, text }) {
    console.log(`[SMTPDriver] Sending email via SMTP...`);
...
class MockEmailDriver {
  async send(email) {
    console.log(`[MockEmailDriver] Mock email would be sent to: ${email.to}`);
    return { id: 'mock-id' };
  }
}
...
export class EmailService {
  ...
  async _initializeDriver() {
    ...
    switch (driverType) {
      ...
      case 'mock':
        console.log("[EmailService] Mock Driver selected.");
        this.driver = new MockEmailDriver();
        break;
